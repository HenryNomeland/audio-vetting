...
def main(page: ft.Page):
    page.add(ft.Container(ft.Text("Syncing network drives to Sqlite database...", size = 20), alignment = ft.alignment.center, width = page.width, height = page.height))
    page.update()
...
def gen_specific_worker_table(workerID = 1):
    if type(workerID) is int:
        filteredSQL = SQLDataTable("sqlite", "audio.db", statement = 
                                f"""
...
def application(page: ft.Page):
    ...
    fileSQL = SQLDataTable("sqlite", "audio.db", statement =
                           """
                           ...
                           LEFT JOIN Workers ON Files.WorkerID = Workers.WorkerID
                           """ )
    folderSQL = SQLDataTable("sqlite", "audio.db",
                             statement =
                             """
                             SELECT Folders.FolderName, Folders.TotalFiles, Folders.FolderPath
                             FROM Folders
                             """)
    workerSQL = SQLDataTable("sqlite", "audio.db",
                             statement = 
                             """
    ...
    vetting_tab = ft.Column([vetting_row, gen_specific_worker_table()], tight = True, scroll = "auto")
    ...
    tabs = ft.Tabs(
        ...
        tabs = [
            ft.Tab(
                text = "Vetting",
                content = ft.Container(content = vetting_tab, alignment = ft.alignment.top_left, padding = 20)
            ),
            ft.Tab(
                text = "All Files",
                content = ft.Container(content = files_tab, alignment = ft.alignment.top_left, padding = 20)
            ),
            ft.Tab(
                text = "All Folders",
                content = ft.Container(content = folders_tab, alignment = ft.alignment.top_center, padding = 20)
            ),
            ft.Tab(
                text = "All Workers",
                content = ft.Container(content = workers_tab, alignment = ft.alignment.top_center, padding = 20)
            )
        ],
        expand = 1
    )
